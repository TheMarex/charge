...
#include "../helper/vector_comparator.hpp"
...
void test_algorithm(
    const Charge &charge, const Charge::Algorithm algorithm,
    const std::vector<std::tuple<common::Coordinate, common::Coordinate>>
        &queries,
    const std::vector<RouteResult> &references) {
    std::vector<RouteResult> results_bi_dijkstra(queries.size());

    std::transform(
        queries.begin(), queries.end(), results_bi_dijkstra.begin(),
        [&charge, algorithm](const auto &query) -> RouteResult {
            auto[start_coord, target_coord] = query;
            auto start = charge.nearest(start_coord);
            auto target = charge.nearest(target_coord);
            return charge.route(algorithm, start.id, target.id).front();
        });

    for (auto index : common::irange<std::size_t>(0, references.size())) {
        auto &result = results_bi_dijkstra[index];
        auto &ref = references[index];
        CHECK(ApproxVector(result.lengths) == ref.lengths);
        CHECK(ApproxVector(result.durations) == ref.durations);
        CHECK(ApproxVector(result.consumptions) == ref.consumptions);
        CHECK(ApproxVector(result.geometry) == ref.geometry);
        CHECK(ApproxVector(result.path) == ref.path);
        CHECK(ApproxVector(result.max_speeds) == ref.max_speeds);
    }
};
...
TEST_CASE("Start server", "[Server]") {
    ...
    std::vector<std::tuple<Coordinate, Coordinate>> queries = {
        {Coordinate::from_floating(0.0, 0.0),
         Coordinate::from_floating(4.0, 1.0)},  // 0->9
        {Coordinate::from_floating(0.0, 1.0),
         Coordinate::from_floating(1.0, 1.0)},  // 5->6
        {Coordinate::from_floating(3.0, 0.0),
         Coordinate::from_floating(3.0, 1.0)},  // 3->8
        {Coordinate::from_floating(3.0, 0.0),
         Coordinate::from_floating(1.0, 1.0)},  // 3->6
        {Coordinate::from_floating(4.0, 1.0),
         Coordinate::from_floating(0.0, 1.0)},  // 9->5
        {Coordinate::from_floating(5.0, 1.0),
         Coordinate::from_floating(4.0, 1.0)},  // 10->9
        {Coordinate::from_floating(2.0, 0.0),
         Coordinate::from_floating(4.0, 0.0)},  // 2->4
        {Coordinate::from_floating(2.0, 0.0),
         Coordinate::from_floating(2.0, 0.0)},  // 2->2
    };

    std::vector<RouteResult> references{
        RouteResult{
            ev::make_constant(1.9, 5),
            {0.0, 0.1, 0.3, 0.6, 1.1, 1.9},
            {4004146.8000000319, 2002073.400000016, 2669431.2000000216,
             1790636.1476587767, 1000884.221782684},
            {0.0, 1.0f, 2.0f, 3.0f, 4.0f, 5.0f},
            {0.0, 111226.3000000009, 222452.6000000018, 444905.2000000036,
             693604.6649526114, 916023.380904319},
            {0, 1, 2, 4, 7, 9},
            ...
            {}},
        RouteResult{
            ev::make_constant(1.4, 3),
            {0.0f, 0.6f, 0.9f, 1.4f},
            {1492196.7897156475, 2669431.2000000216, 2532329.0545668923},
            {0.0, 1.0f, 2.0f, 3.0f},
            ...
            {}},
        RouteResult{
            ev::make_constant(2.2, 5),
            {0.0f, 0.4f, 0.6f, 0.9f, 1.4f, 2.2f},
            { 2002073.4000000164, 2002073.4000000155, 2669431.2000000207, 1790636.1476587767, 500442.1166962503 },
            {0.0f, 1.0f, 2.0f, 3.0f, 4.0f, 5.0f},
            {0.0f, 222452.59375f, 333678.90625f, 556131.5f, 804830.9375f,
             916040.3125f},
            {3, 1, 2, 4, 7, 8},
            ...
            {}},
        RouteResult{
            ev::make_constant(1.4, 4),
            {0.0f, 0.4f, 0.6f, 0.9f, 1.4f},
            { 2002073.4000000164, 2002073.4000000155, 2669431.2000000207, 2532329.0545668923},
            ...
            {}},
        RouteResult{
            ev::make_constant(2.6, 4),
            {0.0f, 1.0f, 1.8f, 2.3f, 2.6f},
            {800707.377426147, 1119147.5922867358, 1601658.7200000132, 2984393.5794312945},
            {0.0f, 1.0f, 2.0f, 3.0f, 4.0f},
            ...
            {}},
        RouteResult{
            ev::make_constant(1.9, 2),
            {0.0f, 1.1f, 1.9f},
            {1091873.6753762988, 1000884.221782684},
            {0.0f, 1.0f, 2.0f},
    ...
    test_algorithm(charge, Charge::Algorithm::FASTEST_BI_DIJKSTRA, queries, references);
    test_algorithm(charge, Charge::Algorithm::MCC_DIJKSTRA, queries, references);
    test_algorithm(charge, Charge::Algorithm::FPC_DIJKSTRA, queries, references);
}
