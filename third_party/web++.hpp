...
namespace WPP {
    ...
    using CallbackFn = std::function<void(Request*, Response*)>;
    ...
    class Server {
        public:
            void get(string, CallbackFn callback);
            void post(string, CallbackFn callback);
            void all(string, CallbackFn callback);
    ...
    void Server::get(string path, CallbackFn callback) {
        Route r = {
             path,
             "GET",
             callback
    ...
    void Server::post(string path, CallbackFn callback) {
        Route r = {
             path,
             "POST",
             callback
    ...
    void Server::all(string path, CallbackFn callback) {
        Route r = {
             path,
             "ALL",
             callback
    ...
    void* Server::main_loop(void* arg) {
        ...
        int yes = 1;
        if (::setsockopt(sc, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(int)) != 0)
        {
            throw WPP::Exception("ERROR on setsockopt");
        }

        if (::bind(sc, (struct sockaddr *) &serv_addr, sizeof(serv_addr)) != 0) {
            auto msg = std::string("ERROR on binding: ") + strerror(errno);
            throw WPP::Exception(msg.c_str());
        }
        ...
        while(running) {
            ...
            tv.tv_sec = 0;
            tv.tv_usec = 100000;
            auto result = select(sc+1, &rfds, NULL, NULL, &tv);
            if (result == 0)
            {
                continue;
            }
            ...
            {
                throw WPP::Exception("ERROR on select");
            }
