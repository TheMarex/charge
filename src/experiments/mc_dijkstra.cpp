...
int main(int argc, char **argv) {
    if (argc < 10) {
        std::cerr << argv[0] << " EXPERIMENT_PATH NUM_RUNS THREADS LOG_PATH "
                                "GRAPH_BASE_PATH CAPACITY_WH CONSUMPTION_SAMPLE_RESOLUTION X_EPS Y_EPS"
                  << std::endl;
        std::cerr << "Example:" << argv[0]
                  << "cache/luxev/random 10 2 results/random_dijkstra data/luxev 16000.0 10.0 0.1 1.0"
                  << std::endl;
        return EXIT_FAILURE;
    }
    ...
    const double sample_resolution = std::stof(argv[7]);
    ...
    auto runner = experiments::make_experiment_runner(
        ev::MCAStarContext{x_eps, y_eps, capacity, graph, reverse_min_duration_graph},
        std::move(queries),
